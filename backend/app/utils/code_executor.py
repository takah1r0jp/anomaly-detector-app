...
def execute_code(code, image_path):
    ...
    # 関数の実行，正常：０，異常：1
    anomaly_score = execute_function_from_code(final_funciton, function_name, image_path, image)
    ...
    if anomaly_score == 0:
        return {"message": "この画像は条件を満たしています"}
    else:
        return {"message": "この画像は条件を満たしていません"}
...
def main():
    image_path = "apple_strawberry.png"
    code = """
    ```python
